...
export const CDK_DEPLOY_DEFAULT_PARAMETERS_FILE_PATH =
  "cdk.parameters.default.env";

export const makeStackParameterPath = (
  cdkAppKey: string,
  cdkEnvKey: string,
  ...paths: string[]
) =>
  "/" +
  ["CDK", pascalCase(cdkAppKey), pascalCase(cdkEnvKey), ...paths]
    .filter((s) => !!s)
    .join("/");

const makeCdkDeployParametersFilePath = (
  cdkAppKey: string,
  cdkEnvKey: string
) => {
  ...
  return path.resolve(
    cwd,
    cdkJson.output || "cdk.out",
    `${cdkAppKey}${cdkEnvKey}.parameters.json`
  );
};
...
export const loadStackParameters = () => {
  ...
  if (!cdkEnvKey) {
    throw Error(
      `Environment Variable not found: ${ENVIRONMENT_VARIABLE_NAME_CDK_ENV_KEY}`
    );
  }
  ...
  const getParametersByPath: (
    ...
    if (ssmResult.NextToken) {
      return [
        ...
        ...(await getParametersByPath(ssmResult.NextToken)),
      ];
    }
...
const getStackTagMappingList = async (
  ...
  ) => Promise<ResourceGroupsTaggingAPI.ResourceTagMappingList> = async (
    paginationToken
  ) => {
...
export const getTagNameCdkEnvKey = (cdkAppKey: string) =>
  cdkAppKey || "CdkEnvKey";
...
export const getStackNamesPerCdkEnvKey = async (
  ...
  const stackNamesPerCdkEnvKey = stackTagMappingList.reduce<{
    [cdkEnvKey: string]: string[];
...
export const loadCdkDeployParametersDefault = () => {
  ...
  if (!defaultParameters) {
    ...
    if (!fs.existsSync(defaultParametersFilePath)) {
      fs.writeFileSync(defaultParametersFilePath, "#REMOVAL_POLICY=retain", {
        encoding: "utf8",
      });
      console.log(
        `file not found: ${defaultParametersFilePath}, created default parameter file.`
      );
    }
    defaultParameters =
      dotenv.config({ path: defaultParametersFilePath }).parsed || {};
  }
...
export const loadCdkDeployParametersFromSsm: (
  ...
  const response = await new AwsSdkSsm(option)
    .getParameter({
      Name: makeStackParameterPath(
        cdkAppKey,
        cdkEnvKey,
        CDK_DEPLOY_PARAMETERS_KEY
      ),
    })
    .promise();
...
export const writeCdkDeployParametersToSsm = async (
  ...
  new AwsSdkSsm(option)
    .putParameter({
      ...
      Tags: overwrite
        ? undefined
        : [{ Key: getTagNameCdkEnvKey(cdkAppKey), Value: cdkEnvKey }],
    })
    .promise()
    .catch((e) => {
      if (e.code === "ParameterAlreadyExists") {
        return writeCdkDeployParametersToSsm(
          cdkAppKey,
          cdkEnvKey,
          deployParameters,
          true,
...
export const writeCdkDeployParametersToLocal = (
  ...
  fs.writeFileSync(filePath, stringify(deployParameters, { maxLength: 4 }), {
    encoding: "utf8",
  });
};
...
export const mergeCdkDeployParameters = (
  ...
  const mergedParameters = Object.keys(
    defaultParameters
  ).reduce<CdkDeployParameters>(
    (payload, key) => ({
      ...payload,
...
export const diffCdkDeployParametersKeys = (
  a: CdkDeployParameters,
  b: CdkDeployParameters
) => {
